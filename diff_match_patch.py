...
class diff_match_patch:
  ...
  def diff_summary(self, text1, text2):
    self.Diff_Timeout = 0
    diffs = self.diff_wordMode(text1, text2)
    ...
    for i, (diff_type, text) in enumerate(diffs):
      ...
      next_diff_text = None
      if i+1 < len(diffs):
        next_diff_type, next_diff_text = diffs[i+1]

      if diff_type == self.DIFF_EQUAL:
        summary.append({'type': 'equal', 'text': [text]})
      elif diff_type == self.DIFF_DELETE:
        if next_diff_type == self.DIFF_INSERT:
          summary.append({'type': 'replace', 'text': [text, next_diff_text]})
          del diffs[i+1]
        else:
          summary.append({'type': 'delete', 'text': [text, '']})
      elif diff_type == self.DIFF_INSERT:
        if next_diff_type == self.DIFF_DELETE:
          # this caase should not really happen since repalces are always deletes follwed by insert, not the other way round
          summary.append({'type': 'replace', 'text': [next_diff_text, text]})
          del diffs[i+1]
        else:
          summary.append({'type': 'insert', 'text': ['', text]})
